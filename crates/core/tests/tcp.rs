...
fn parse_linux_proc_tables() {
  ...
  sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode\n\
   ...
   1: 0100007F:1F91 00000000:0000 01 00000000:00000000 00:00000000 00000000  1000        0 12346 1 0000000000000000 100 0 0 10 0\n";
    ...
    let parsed = sidecar_core::runtime::tcp::parse_linux_tcp_table(table, &inodes).unwrap();
...
sidecar  1234 fire   12u  IPv6 0x5678      0t0  TCP [::1]:49223 (LISTEN)\n";
    let parsed = sidecar_core::runtime::tcp::parse_lsof_listeners(text);
...
#[test]
fn windows_network_order_port() {
    assert_eq!(sidecar_core::runtime::tcp::windows_port(0x901F), 8080);
}

#[cfg(any(target_os = "linux", target_os = "macos", windows))]
#[test]
fn discover_current_process() {
    let listener = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let expected = listener.local_addr().unwrap();
    let listeners = sidecar_core::tcp_listeners_for_pid(std::process::id()).unwrap();
