...
if [ "${SMOKE_LATEST:-}" = "1" ]; then
  ...
  sh "$ROOT/manage.sh" install --channel "$CHANNEL" --install-root "$SIDECAR_INSTALL_ROOT/latest-smoke"
  "$SIDECAR_LOCAL_BIN_DIR/sidecar" --version
  "$SIDECAR_LOCAL_BIN_DIR/sidecar" doctor --config "$ROOT/examples/minimal.toml"
  sh "$ROOT/manage.sh" uninstall --install-root "$SIDECAR_INSTALL_ROOT/latest-smoke"
